            ...
            <tbody>
              @for (r of filteredReceivables(); track r.id) {
                <tr [class.row-overdue]="r.status === 'ATRASADA'">
                  ...
                  <td class="text-center">
                    @if (r.recurrence === 'INSTALLMENT') {
                      <div style="display: flex; gap: 6px; justify-content: center; align-items: center;">
                        <tc-badge tone="info">Parcelado</tc-badge>
                        <tc-badge tone="accent">{{ getInstallmentLabel(r) }}</tc-badge>
                      </div>
      ...
      @if (!hasCategories()) {
        ...
        <form [formGroup]="receivableForm" (ngSubmit)="saveReceivable()">
          ...
          <tc-select
            ...
            [fullWidth]="true"
          ></tc-select>
          @if (receivableForm.get('recurrence')?.value === 'INSTALLMENT') {
            <tc-input
              type="number"
...
export class AccountsReceivablePageComponent implements OnInit {
  ...
  getInstallmentLabel(r: ReceivableAccount): string {
    if (r.recurrence === 'INSTALLMENT' && r.installmentNumber && r.totalInstallments) {
  ...
  openEditModal(receivable: ReceivableAccount): void {
    ...
    this.receivableForm.patchValue({
      ...
      totalInstallments: receivable.totalInstallments ?? null,
    });
    this.isFormModalOpen.set(true);
  }
